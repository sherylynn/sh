...
start_chroot_container() {
  ...
  # 配置DNS解析器
  log_debug "配置DNS解析器 (resolv.conf)..."
  # 优先从主机(Termux)复制resolv.conf, 这是最可靠的DNS来源
  if [ -f "$PREFIX/etc/resolv.conf" ]; then
    log_debug "从 $PREFIX/etc/resolv.conf 复制DNS配置..."
    # 先删除chroot中可能存在的无效符号链接或旧文件
    sudo rm -f "${CHROOT_DIR}/etc/resolv.conf"
    # 复制配置文件
    sudo cp "$PREFIX/etc/resolv.conf" "${CHROOT_DIR}/etc/resolv.conf"
    # 确保文件权限正确
    chroot_exec -u root chmod 644 /etc/resolv.conf
    is_ok "fail" "done"
  else
    # 如果主机文件不存在，则回退到硬编码的公共DNS
    log_warn "Termux的resolv.conf未找到，回退到公共DNS..."
    chroot_exec -u root bash -c 'rm -f /etc/resolv.conf && echo "nameserver 223.5.5.5" > /etc/resolv.conf'
    chroot_exec -u root bash -c 'echo "nameserver 114.114.114.114" >> /etc/resolv.conf'
    is_ok "fail" "done"
  fi
