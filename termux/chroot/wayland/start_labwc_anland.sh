...
MODE_FILE=${NEWHOME_WAYLAND_MODE_FILE:-/root/.config/newhome-wayland-mode}
WAYLAND_MODE=${NEWHOME_WAYLAND_MODE:-}
[ -n "$WAYLAND_MODE" ] || WAYLAND_MODE=$(head -n 1 "$MODE_FILE" 2>/dev/null || true)
WAYLAND_MODE=${WAYLAND_MODE:-direct}
