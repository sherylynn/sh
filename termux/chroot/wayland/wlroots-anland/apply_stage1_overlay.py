#!/usr/bin/env python3
"""Apply the stage-1 Anland backend to an unpacked wlroots 0.18.2 tree.

This is deliberately an exact-source transformer instead of a fuzzy patch:
Debian 13's wlroots ABI is pinned, and every edit validates its anchor before
writing.  Stage 1 provides backend/output discovery and reconnect scaffolding;
it intentionally does NOT claim that Anland consumer DMA-BUF presentation is
implemented yet.
"""
...
/**
 * Create an Anland producer backend.
 *
 * The display daemon socket is read from ANLAND_SOCKET, falling back to
 * /tmp/anland/display_daemon.sock. The first implementation targets the
 * Anland 5.13 producer protocol used by lfdevs/anland-termux 5.13.3.
 */
...
struct wlr_anland_backend *anland_backend_from_backend(struct wlr_backend *wlr_backend) {
    ...
    return wl_container_of(wlr_backend, (struct wlr_anland_backend *)0, backend);
}
...
/* Stage 1 uses wlroots' CPU-capable fallback renderer only. The final backend
 * changes this contract when the consumer-owned DMA-BUF allocator/presentation
 * path lands. Do not describe this stage as zero-copy. */
...
struct wlr_backend *wlr_anland_backend_create(struct wl_event_loop *loop) {
    ...
    backend->reconnect_timer = wl_event_loop_add_timer(loop, reconnect_timer, backend);
    if (backend->reconnect_timer == NULL) {
        disconnect(backend->display);
        free(backend->socket_path);
        free(backend);
...
static bool output_commit(struct wlr_output *wlr_output,
    ...
    /* A compositor submitting a rendered buffer has reached the stage-3
     * boundary. Reject it rather than silently dropping frames or calling
     * trigger_refresh() for a buffer Android never received. */
    if (state->committed & WLR_OUTPUT_STATE_BUFFER) {
        wlr_log(WLR_ERROR,
            "Anland stage1 has no DMA-BUF presentation path yet; refusing buffer commit");
        return false;
    }
    return true;
}
...
struct wlr_output *anland_backend_add_output(struct wlr_anland_backend *backend) {
    ...
    wlr_output_set_description(&output->wlr_output,
        "Anland Android display (stage1 discovery backend)");

    int refresh_mhz = refresh > 0 ? refresh : 60000;
    output->frame_delay_ms = 1000000 / refresh_mhz;
    if (output->frame_delay_ms < 1) {
        output->frame_delay_ms = 1;
    }
    output->frame_timer = wl_event_loop_add_timer(backend->event_loop,
        signal_frame, output);
    if (output->frame_timer == NULL) {
        wlr_output_finish(&output->wlr_output);
        free(output);
        return NULL;
    }
...
def main() -> None:
    ...
    if "0.18.2" not in meson.read_text():
        raise RuntimeError("stage1 overlay only supports wlroots 0.18.2")
    ...
    replace_once(
        root / "backend/meson.build",
        "subdir('headless')\n",
        "subdir('headless')\nsubdir('anland')\n",
    )

    replace_once(
        root / "backend/backend.c",
        "#include <wlr/backend/headless.h>\n",
        "#include <wlr/backend/headless.h>\n#include <wlr/backend/anland.h>\n",
    )

    marker = "static struct wlr_backend *attempt_headless_backend(struct wl_event_loop *loop) {"
    backend_c = root / "backend/backend.c"
    text = backend_c.read_text()
    if marker not in text:
        raise RuntimeError("headless backend anchor not found")
    helper = '''static struct wlr_backend *attempt_anland_backend(struct wl_event_loop *loop) {\n\treturn wlr_anland_backend_create(loop);\n}\n\n'''
    backend_c.write_text(text.replace(marker, helper + marker, 1))
    ...
    print(f"stage1 Anland backend overlay applied to {root}")
